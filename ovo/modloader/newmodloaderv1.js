(function () {
    ...
    class ModLoader {
        ...
        async #init() {
            this.mods = await fetch(this.getModDirectory() + version + ".json").then(res => res.json());
        ...
        loadModURL(url, local = false, notify = true) {
            if (local) {
                url = this.getModDirectory() + version + "/" + url;
            }
        ...
        loadModJSON(key, json, notify = true) {
            const url = this.getModDirectory() + version + "/" + json.url;
